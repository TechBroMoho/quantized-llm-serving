...
- **Validation (commit the results to `results/validation/`):**
  1. **Accuracy:** the mock requests 200ms TTFT and 20ms ITL, but the reference is its realized monotonic handler-start-to-first-text-write interval and consecutive text-write intervals (recorded immediately after `response.write()` returns). Each matched request must have TTFT, E2E, TPOT, and every ITL within ±5% of the observed server reference. TTFT/E2E references use the client request-start timestamp so connection/setup time is included; handler-relative medians are descriptive only. Save the individual server write timestamps alongside client records; configured delays are not treated as ground truth. The stream includes an empty text chunk before generated text and a final usage-only event. A deliberately broken timer must make this test fail.
